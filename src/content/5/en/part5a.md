...
In the last two parts, we mainly concentrated on the backend; the frontend does not yet support the user management we implemented to the backend in part 4.


At the moment, the frontend shows existing notes, and allows users to change the state of a note from 'important' to 'not important' and vice versa. New notes cannot be added anymore, as the backend now expects a token, verifying user identity (from the changes we made in part 4). 
